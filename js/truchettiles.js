var screenHeight; // Screen height multiplied by devicePixelRatio.curY
...
const svgArray = []; // Holds Truchet Tile SVGs.
...
window.addEventListener('load', function(event){
  ...
  if (truchetTiles.hasAttribute('data-style')) {
    tilesStyle = truchetTiles.getAttribute('data-style')
  }
  if (truchetTiles.hasAttribute('data-color-1')) {
    tilesColor1 = truchetTiles.getAttribute('data-color-1')
  }
  if (truchetTiles.hasAttribute('data-color-2')) {
    tilesColor2 = truchetTiles.getAttribute('data-color-2')
  }
  if (truchetTiles.hasAttribute('data-color-3')) {
    tilesColor3 = truchetTiles.getAttribute('data-color-3')
  }
  if (truchetTiles.hasAttribute('data-outline-thickness')) {
    tilesThickness = truchetTiles.getAttribute('data-outline-thickness')
  }
  if (truchetTiles.hasAttribute('data-size')) {
    tilesSize = truchetTiles.getAttribute('data-size')
  }
  tilesLoad()
});

function tilesSetStyle(input){
  tilesStyle = input
}

function tilesSetColor1(input){
  tilesColor1 = input
}

function tilesSetColor2(input){
  tilesColor2 = input
}

function tilesSetColor3(input){
  tilesColor3 = input
}

function tilesSetThickness(input){
  tilesThickness = input
}

function tilesSetSize(input){
  tilesSize = input
}
...
document.body.addEventListener('mousemove',function(e) {
  ...
  let firstX = (e.clientX * window.devicePixelRatio) * scrollbar_adjustment
  let firstY = (e.clientY * window.devicePixelRatio)
  let x = Math.floor(firstX / tileDistance)
  let y = Math.floor(firstY / tileDistance)
  if(x != curX || y != curY){
    let style = document.getElementById('truchet-tiles').getAttribute('data-style')
    if(style == '1' || style == '2'){ // This is for the two styles that don't have 4 unique rotations
