var screenHeight; // Screen height multiplied by devicePixelRatio.
var screenWidth; // Screen width multiplied by devicePixelRatio.
...
var tilesStyle = '9';
var tilesColor1 = '#DEE1E6';
var tilesColor2 = '#F1F3F4';
var tilesColor3 = '#292F33';
var tilesThickness = '40';
var tilesSize = '12';

// These values are maintained each time the canvas is drawn.
var liveTilesStyle = null;
var liveTilesDistance = null;

// Base tile SVGs before color and stroke width are applied.
const svgArray = [];
svgArray[1] =   '<g id="group">' +
...
window.addEventListener('load', function(event){
  ...
  if (truchetTiles.hasAttribute('data-color-1')) {
    tilesSetColor1(truchetTiles.getAttribute('data-color-1'), false);
  }
  if (truchetTiles.hasAttribute('data-color-2')) {
    tilesSetColor2(truchetTiles.getAttribute('data-color-2'), false);
  }
  if (truchetTiles.hasAttribute('data-color-3')) {
    tilesSetColor3(truchetTiles.getAttribute('data-color-3'), false);
  }
...
function tilesSetStyle(input, load){
  ...
  for (let i = 0; i < styleArray.length; i++) {
    ...
    if(numStyle < 1 || numStyle > 15){
      console.error('Style \'' + input + '\' is out of the range 1-15. Using last successful style value or default style value.');
      withinRange = false;
    }
  }
  if(withinRange){
...
function tilesSetColor1(input, load){
  ...
  let validColor = styleTest.color !== '';
  if(validColor){
    tilesColor1 = input;
  }
  else{
    console.error('Color \'' + input + '\' is not a valid color. Using last successful color1 value or default color1 value.');
  }
  if(load){
    tilesLoad();
  }
...
function tilesSetColor2(input, load){
  ...
  if(validColor){
    ...
    console.error('Color \'' + input + '\' is not a valid color. Using last successful color2 value or default color2 value.');
  }
  if(load){
    tilesLoad();
  }
...
function tilesSetColor3(input, load){
  ...
  if(validColor){
    ...
    console.error('Color \'' + input + '\' is not a valid color. Using last successful color3 value or default color3 value.');
  }
  if(load){
    tilesLoad();
  }
...
function tilesSetThickness(input, load){
  ...
  if(numThickness < 0 || numThickness > 100){
    console.error('Thickness \'' + input + '\' is out of the range 0-100. Using last successful thickness value or default thickness value.');
    withinRange = false;
  }
  if(withinRange){
...
function tilesSetSize(input, load){
  ...
  if(numSize < 1 || numSize > 50){
    console.error('Size \'' + input + '\' is out of the range 1-50. Using last successful size value or default size value.');
    withinRange = false;
  }
  if(withinRange){
    tilesSize = input
...
// Helper function to get a random tile value.
...
function tilesLoad() {
  const styleArray = tilesStyle.split('&');
    ...
    if(newStyle || newDistance){
      ...
      for (let i = 0; i < numSquares; i++) {
        ...
        for (let j = 0; j < numSquares; j++) {
          imgGridArray[i][j] = tilesGetRandomInt(totalImages);
        }
      }
    }
